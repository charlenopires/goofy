...
impl<'a> DatabaseOperations for TransactionQueries<'a> {
    // Session operations
    fn create_session(&self, session: &Session) -> Result<()> {
        self.tx.execute(
            "INSERT INTO sessions (
    ...
    fn delete_session(&self, id: &str) -> Result<()> {
        self.tx.execute("DELETE FROM sessions WHERE id = ?1", params![id])?;
        Ok(())
    }
    ...
    fn delete_message(&self, id: &str) -> Result<()> {
        self.tx.execute("DELETE FROM messages WHERE id = ?1", params![id])?;
        Ok(())
    }
    ...
    fn delete_file(&self, id: &str) -> Result<()> {
        self.tx.execute("DELETE FROM files WHERE id = ?1", params![id])?;
        Ok(())
    }
}